...
import type { ReportFormData, Category } from '../lib/ui-types'
...
export default function ReportModal({ open, onClose, onSubmit }: ReportModalProps) {
  ...
  const canNext =
    ...
    : step === 1 ? form.title.trim().length >= 10 && form.description.trim().length >= 20 && form.location.trim().length >= 3
    : true
                ...
                {CATEGORIES.map((cat) => (
                  <button
                    ...
                    className={`flex items-center gap-2 p-3 rounded-xl text-left text-sm font-medium border transition-all ${
                      form.category === cat.value ? `${cat.color} border-current` : 'border-border text-foreground-2 hover:border-foreground-2 hover:text-foreground bg-surface-2'
                    }`}
                  >
            ...
            <div className="flex flex-col gap-3">
              ...
              <div>
                ...
                <textarea value={form.description} onChange={(e) => set('description', e.target.value)}
                  placeholder="Describe con el mayor detalle posible lo que ocurrió..." rows={3}
              ...
              <div>
                <label className="block text-xs text-foreground-2 mb-1.5">Evidencia (URL imagen, opcional)</label>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="rounded-xl border border-border bg-surface-2 p-4 space-y-2 text-sm">
                {[
                  ...
                  ['Título', form.title],
                  ['Ubicación', `${form.location} — ${form.region}`],
              ...
              <p className="text-xs text-muted">Al publicar aceptas que el contenido es verídico.</p>
            </div>
          )}
        </div>
