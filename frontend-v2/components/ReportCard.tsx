...
import { getCategoryMeta, timeAgo } from "@/lib/utils";
...
export default function ReportCard({ report, onVote }: ReportCardProps) {
  ...
  return (
    <article className="group flex gap-4 p-4 rounded-xl border border-border bg-surface hover:border-border hover:bg-surface-2 transition-all duration-200">
      {/* Vote column */}
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-sm text-foreground-2 leading-relaxed line-clamp-2 mb-3">
          {report.description}
        </p>
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs text-muted">
          ...
          <span>
            {report.isAnonymous ? "Anónimo" : `por ${report.author}`}
          </span>
        </div>
      </div>
    </article>
