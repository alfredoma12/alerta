...
import type { Zone, TopAccount } from '../lib/ui-types'
  ...
  return (
    <aside className="flex flex-col gap-4">
      <div className="rounded-xl border border-border bg-surface p-4">
        <h3 className="font-heading font-semibold text-foreground text-sm mb-3">🔥 Zonas más activas</h3>
        {zones.length === 0 ? (
          <p className="text-xs text-muted">Sin denuncias registradas por ahora.</p>
        ) : (
      ...
      <div className="rounded-xl border border-border bg-surface p-4">
        <h3 className="font-heading font-semibold text-foreground text-sm mb-3">⚠️ Más denunciados</h3>
        {accounts.length === 0 ? (
          <p className="text-xs text-muted">Aún no hay publicaciones para mostrar.</p>
        ) : (
          <ul className="space-y-2.5">
            {accounts.map((acc) => (
              <li key={acc.handle} className="flex items-center justify-between gap-2">
                <div className="min-w-0">
                  <p className={`text-sm font-medium truncate ${platformColor[acc.platform] ?? 'text-foreground-2'}`}>{acc.handle}</p>
                  <p className="text-xs text-muted capitalize">{acc.platform}</p>
                </div>
                <span className="flex-shrink-0 text-xs font-semibold text-primary bg-primary/10 px-2 py-0.5 rounded-full">{acc.reports}</span>
              </li>
            ))}
          </ul>
        )}
      ...
      <div className="rounded-xl border border-primary/20 bg-primary/5 p-4">
        <h3 className="font-heading font-semibold text-foreground text-sm mb-1.5">Denuncia sin exponer tu identidad</h3>
        <p className="text-xs text-foreground-2 mb-3">Puedes publicar de forma anónima. No guardamos tu IP ni datos personales.</p>
        <button
          onClick={onDenunciar}
