...
const statusMeta = {
  verified: { icon: CheckCircle, label: 'Verificado por comunidad', color: 'text-green' },
  pending: { icon: AlertCircle, label: 'En revision', color: 'text-amber' },
  rejected: { icon: Shield, label: 'Rechazado', color: 'text-muted' },
}

const verificationMeta = {
  unverified: { label: 'No verificado', color: 'text-foreground-2 border-border bg-surface-2' },
  review: { label: 'En revision', color: 'text-amber border-amber/30 bg-amber/10' },
  community_verified: { label: 'Comunidad verificada', color: 'text-green border-green/30 bg-green/10' },
}
    ...
    <article className="group flex gap-4 p-4 rounded-xl border border-border bg-surface hover:bg-surface-2 transition-all duration-200">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex flex-wrap items-center gap-2 mb-2">
          ...
          <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs border ${verificationMeta[verificationState].color}`}>
            {verificationMeta[verificationState].label}
          </span>
        </div>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          <button className="text-xs px-2.5 py-1 rounded-md border border-border text-foreground-2 hover:text-foreground hover:border-foreground-2 transition-all">
