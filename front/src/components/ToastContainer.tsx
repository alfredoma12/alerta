...
import type { ToastData } from '../lib/ui-types'
  ...
  return (
    <div className="fixed bottom-6 right-4 z-[60] flex flex-col gap-2 items-end">
      {toasts.map((toast) => {
        ...
        return (
          <div key={toast.id} className={`flex items-start gap-3 w-80 p-4 rounded-xl border ${m.bg} bg-surface shadow-xl animate-slide-up`}>
            ...
            <button onClick={() => onRemove(toast.id)} className="text-muted hover:text-foreground transition-colors flex-shrink-0">
              <X className="w-4 h-4" />
            </button>
          </div>
        )
